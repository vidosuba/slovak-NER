...
def get_structured_docs_2(in_path, out_path):
    ids = []
    d = 0
...
def pick_random_docs2(in_path, out_path, n_to_pick):
...
# def tokenize(docs):
#
#     text = """Prvotný objav kyseliny chlorovodíkovej sa pripisuje alchymistovi Abú Músá Džábir ibn Hajján (pribl. 800 n. l.)."""
#     x = nltk.tokenize.word_tokenize(text, language='czech', preserve_line=False)
#     p_tokenizer = nltk.tokenize.punkt.
#     print(p_tokenizer._tokenize_words(text))
#     print(x)
#     # for token in doc:
#     #     print(token.text)
...
def main():
    ...
    #ids = get_structured_docs_2(text_extracted_path, text_valid)               # STEP 1: get valid docs - long enough from parsed dataset (already new enough)

    #picked_docs = pick_random_docs2(text_valid, text_picked, n_docs)           # STEP 2: sampling from valid docs

                                                                                # STEP 3: manually clear sampled docs

    #save_to_jsonl('text_picked/20211216-171112', 'text_jsonl')                 # STEP 4: create jsonl for prodigy
